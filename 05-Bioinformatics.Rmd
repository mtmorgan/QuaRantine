...
Note that the DNA alphabet contains 'ambiguity' letters, corresponding to base seuqencs that were uncertain, e.g., 'A' or 'T'. Check out the help page for `?translate` to see options for translating ambiguity codes. How many unique protein sequences?
...
The `names(aa)` are misleading -- these are the identifiers of a representative accession, but several accessions may map to the same identifier. The following code creates a table that indicates which cluster each original identifer belongs to; the names of `aa` are updated accordingly.
