...
`%in%`: a _binary operator_

- is each of the vector elements on the left-hand side _in_ the set of elements on the right hand side
...
#### Saving individual _R_ objects {-}

_R_ supports storing a single _R_ object as an `.rds` file. For example, the code below saves the `day1_activity` vector to an `.rds` file. The `saveRDS()` function is the workhorse in this case and the `setwd()`, `getwd()`, and `file.path()` commands allow us to conveniently specify a name and location for the data file:
...
#### Loading individual _R_ objects {-}
