...
- The columns of a data frame contain data fields.
  ...
  - `activity`, `duration`, `is work?`, `level`, `date`, etc.
...
Row deletion using row indices is more straightforawrd. Just use negative subsetting  ....

```{r}
# suppose the study has some dropouts ....
dropouts = c(2,3)

df # referesh my memory about df contents ....

df <- df[-dropouts, ]

df # items 2 and 3 are dropped!
```
...
- Remember use of `diff()`
    ...
    ## note 'diff()' what about an implicit '0' at the start of a sequence?
    diff( c(0, x) )
    ```
