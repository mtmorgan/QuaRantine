...
trailing_difference <- function(x, n_days = 7) {
    ...
    stats::filter(diff, average_weights, sides = 1)
}
```
...
- Document the function using `roxygen` formatting. This involves placing special comment characters `#'` at the start of lines, and using 'tags' that describe different parts of the function. One can guess at the meaning of most of the tags. We use the special tags `@importFrom` to tell _R_ that we want to use the `filter` from the stats package, and `@export` to indicate that the function is meant for the 'end user' (i.e., us!).
